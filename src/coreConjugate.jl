...
# Also, note that the sufficient statistics of tib and tjb are not updated in this procedure.
    ...
    for ks = 1:ns
        ...
        if k!=i && k!=j
            if zsa[k]==cia
                ni -= 1; Theta_remove!(tia,x[k])
            else
                nj -= 1; Theta_remove!(tja,x[k])
            end
...
function split_merge!(x,z,zs,S,theta,list,N,t,H,a,b,log_v,n_split,n_merge)
    ...
    if ci0==cj0  # propose a split
        ...
        log_prior_b = log_v[t+1] + lgamma(ni+b)+lgamma(nj+b)-2*lgamma(a)
        log_prior_a = log_v[t] + lgamma(ns+b)-lgamma(a)
        ...
        # compute acceptance probability
        log_prior_b = log_v[t-1] + lgamma(ns+b)-lgamma(a)
        log_prior_a = log_v[t] + lgamma(ni+b)+lgamma(nj+b)-2*lgamma(a)
